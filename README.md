...
### 🗓 활동 히트맵
- GitHub 잔디 스타일로 일별 토큰 사용량을 시각화 (5단계 강도: 흰색 → 연한 주황 → 진한 주황)
- 셀 호버 시 툴팁으로 총 토큰 / 입력 토큰 / 출력 토큰 / 세션 수 상세 표시
- 미래 날짜는 "예정" 표시로 구분
- 월 레이블 및 요일 레이블 포함
- 하단에 "적음 ~ 많음" 강도 범례 표시
- 열람 시 오늘 날짜 셀로 자동 스크롤

### 📈 요약 통계 (4개 카드)
- **연속 사용일** — 현재 스트릭 및 역대 최고 스트릭
- **이번달 누적** — 이번 달 토큰 수 (보조로 올해 누적 토큰 표시)
- **오늘 세션** — 오늘 사용 토큰 및 오늘 세션 횟수
...
### 헤더 요약 카드
- 히어로 카드에서 **오늘** / **최근 7일** / **전주 대비 증감률(%)** 즉시 확인
- 상단 바에 OAuth 연결 상태 실시간 표시 (연결됨 / 연결 안됨)
- 상단 바에 현재 세션 사용률 % 상시 표시
- 메뉴바 트레이 아이콘에 현재 세션 사용률 % 표시
...
Claude Code에 로그인된 상태라면 OAuth 토큰을 자동 감지해 Anthropic의 실시간 사용량 데이터를 추가로 불러옵니다.
...
4. 터미널에서 `claude` 로그인이 되어 있으면 플랜 사용 한도 패널이 자동 활성화됩니다 *(수동 토큰 설정은 선택)*
...
Claude Code에 로그인된 상태라면 토큰을 자동으로 불러옵니다. 탐색 순서는 다음과 같습니다.

1. 환경 변수 `ANTHROPIC_OAUTH_ACCESS_TOKEN`
2. 프로젝트 루트의 `.env` 파일
3. `~/.claude/.credentials.json`
4. macOS 키체인 (`Claude Code-credentials` 항목)

| 변수 | 필수 여부 | 설명 |
|------|----------|------|
| `ANTHROPIC_OAUTH_ACCESS_TOKEN` | 선택 | 세션/주간 한도 실시간 사용률 표시 활성화 |
| `CLAUDE_DATA_DIR` | 선택 | Claude 데이터 디렉터리 경로 재정의 (기본 `~/.claude/projects`) |
...
- 히트맵은 현재 연도(1월~12월) 범위만 표시 (이전 연도 조회 불가)
